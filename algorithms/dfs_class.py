...
class DfsPuzzle:
    ...
    def print_styled(self, pop_n, exp_n, gen_n, max_fs, curr_d, cost, soln_moves):
        # print("\nSolution path: \n")
        # if len(soln_moves) > 20:
        #     for state, moved_value, action in soln_moves[::]:
        #         print(f"({state[0]}, {state[1]}, {state[2]}, {state[3]}, {state[4]}, {state[5]}, {state[6]}, {state[7]}, {state[8]}) -> {moved_value} {action}")
...
if __name__ == "__main__":
    ...
    puzzle = DfsPuzzle(initial_state, goal_state)
